...
export class ArlasWuiComponent implements OnInit, AfterViewInit {
  ...
  public ngOnInit() {
    ...
    if (this.arlasStartUpService.shouldRunApp && !this.arlasStartUpService.emptyMode) {
      ...
      this.mapDataSources = this.mapglContributors.map(c => c.dataSources).length > 0 ? this.mapglContributors.map(c => c.dataSources).reduce((set1, set2) => new Set([...set1, ...set2])) : new Set();
