...
export class CustomConfigListComponent implements OnInit {
  ...
  public applyInitStyle(){
    ...
    const style: CustomList = {};
    style.columns = this.customListService.rightListContributors[0].fieldsList;
    style.keyToColors = this.colorService.colorGenerator.keysToColors;

    this.customListService.applyListStyle(style);
  };
    ...
    this.persistenceService.list(this.ZONE, this.configPageSize, this.configPageNumber + 1, 'desc')
      .pipe(map(data => {
        if (data.data !== undefined) {
          console.log(data.data.map(d => this.computeData(d)));
